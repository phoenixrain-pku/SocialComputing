...
3. 由 $1$ 和 $2$, 不难想象, 每个人在每个资源点上会有一个"个人水平值"与"真实收益值", "个人水平值"只与个体有关, "真实收益值"与群体有关. 
   ...
   $$
   v_{i,j}(t) =a_i \int_{t=0}^T f_j(t){\rm d}t.
   $$
   ...
   $$
   w_{i,j}(t) = v_{i,j}(t)\cdot\frac{v_{i,j}(t)}{\sum_{i\in S}v_{i,j}(t)}.
   $$
...
该模型中的无向图为 $G=\lang V,E\rang$, 其中 $|V|=n$; 社会中人的集合为 $S$, 其中 $|S|= m $; 社会中人的资源获取能力为 $m$ 维向量 $ A=(a_0,a_1,\cdots, a_{n-1})$. 演化过程是离散的, 即 $t = 0,1,2,\cdots$. 演化规则如下:

1. 第 $0$ 轮: 对图 $G$ 进行随机的初始化操作, 随机选择一定数量的节点对 $(V_i,V_j)$ 连边; 对 $S$ 进行初始化操作, 给每个人在 $V$ 中选择一个初始资源点. 对 $A$ 进行初始化操作, 以 $(0,1)$ 上的正态分布对每一位进行独立赋值, 即 $a_i\sim N(0,1)$. 对所有的节点 $V_j$, 定义单个人的资源获取函数都为满足 $\mu=1,\sigma=0.5$ 的 sigmoid 函数. 这是一个积分有限且满足先增后减的函数. 其函数形式为:
   $$
...
3. 第 $T$ 轮: 每个人在第 $T-1$ 轮的基础上进行短视贪心搜索, 在其所有可达节点中选择一个预估收益最大的节点, 并转移到该节点上进行工作. 但注意: 转移之后真实的收益和预估收益可能不同.
   ...
   他在第 $T$ 轮如果选择节点 $V_i$ 进行工作, 预估的收益应为:
   $$
   \overline w_{i,j}(T)=\frac{\sum_{t=0}^{t_{i,j}(T-1)+1}a_if(t)}{{\sum_{i\in S}v_{i,j}(T-1)}}.
   $$
   在计算预估收益时, $P_i$ 只会考虑该节点现有的总资源, 而不会考虑下一轮可能带来的资源增加. 因此, 当所有人做完选择 (即按照演化规则, 在所有可达节点中选择了预期收益最大的), $P_i$ 在$ V_j$ 真实的收益应为:
   $$
